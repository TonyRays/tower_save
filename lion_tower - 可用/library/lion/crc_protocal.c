...
{
    // frame head
    tx_buffer[UART_MSG_OFFSET_SOF]             =  sof;
    tx_buffer[UART_MSG_OFFSET_DATA_LENGTH]     =  data_length & 0xff;
    tx_buffer[UART_MSG_OFFSET_DATA_LENGTH + 1] =  data_length >> 8;
    tx_buffer[UART_MSG_OFFSET_SEQ]             =  seq;
    append_crc8_check_sum(tx_buffer, UART_MSG_OFFSET_CRC8 + 1);
    
    // cmd id
    tx_buffer[UART_MSG_OFFSET_CMD_ID]          =  cmd_id & 0xff;
    tx_buffer[UART_MSG_OFFSET_CMD_ID + 1]      =  cmd_id >> 8;
    
    // data
    memcpy(tx_buffer + UART_MSG_OFFSET_DATA, data, data_length);
    
    // frame tail
    append_crc16_check_sum(tx_buffer, UART_MSG_OFFSET_DATA + data_length + 2);
    
    return (UART_MSG_OFFSET_DATA + data_length + 2);
}
