...
float pid_calc(pid_t *pid, float get, float set)
{
    ...
    if (pid->err_mode == COMMON_ERR)
        pid->err[NOW] = set - get;
    else if (pid->err_mode == ANGLE_ERR)
        pid->err[NOW] = angle_error_calc(set, get);
    ...
    float kd)
{
  ...
  pid->f_param_init(pid, pid_mode, err_mode, maxout, intergral_limit, kp, ki, kd);
...
pid_t pid_wheel_speed[4] = PID_PARAM_DEFAULT;
pid_t pid_chassis_angle  = PID_PARAM_DEFAULT;
pid_t pid_feed_position  = PID_PARAM_DEFAULT;
pid_t pid_feed_speed     = PID_PARAM_DEFAULT;
pid_t pid_friction_speed[2] = PID_PARAM_DEFAULT;
